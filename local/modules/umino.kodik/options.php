...
$aTabs = [
    ...
    [ // Основные настройки
        'DIV'     => 'edit1',
        'TAB'     => Loc::getMessage('UMINO_KODIK_OPTIONS_TAB_GENERAL'),
        'TITLE'   => Loc::getMessage('UMINO_KODIK_OPTIONS_TAB_GENERAL'),
        'OPTIONS' => [
            [
                'api_token',
                Loc::getMessage('UMINO_KODIK_OPTIONS_API_TOKEN'),
                '2dada2cef52fa5c7f93ba82a5ee32e39',
                ['text', 32]
            ],
            [
                'api_url',
                Loc::getMessage('UMINO_KODIK_OPTIONS_API_URL'),
                'https://kodikapi.com/',
                ['text', 32]
            ],
            [
    ...
    [ // Настройки заполнения
        'DIV'     => 'edit2',
        'TAB'     => Loc::getMessage('UMINO_KODIK_OPTIONS_TAB_FILLING'),
...
?>
    <form action="<?= $APPLICATION->getCurPage(); ?>?mid=<?=$module_id; ?>&lang=<?= LANGUAGE_ID; ?>" method="post">
        ...
        $tabControl->buttons();
        ?>
        <input type="submit" name="apply" value="<?= Loc::GetMessage('UMINO_KODIK_OPTIONS_INPUT_APPLY'); ?>" class="adm-btn-save" />
        <input type="submit" name="default" value="<?= Loc::GetMessage('UMINO_KODIK_OPTIONS_INPUT_DEFAULT'); ?>" />
    </form>
